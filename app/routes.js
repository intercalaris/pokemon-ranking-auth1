...
app.post('/messages', (req, res) => {
  db.collection('messages').findOne({ name: req.body.name }, (err, existingPokemon) => {
    if (err) {
      console.log(err);
      return res.status(500).send('Server error');
    }
    if (existingPokemon) {
      return res.status(400).send('This Pokemon already exists!');
    }

    db.collection('messages').save(
      { name: req.body.name, img: req.body.img, thumbUp: 0 },
      (err, result) => {
        if (err) {
          console.log(err);
          return res.status(500).send('Server error');
